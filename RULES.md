...
- **R27** — **Every session ships per-byte starter scaffolds.** For session N with K bytes, create `dags/s<N>/s<N>_task1.py … s<N>_task<K>.py`, each with full boilerplate through `@dag(...)` + `def pipeline():` + `pipeline()`, `dag_id="s<N>_task<k>"`, a one-line docstring naming the byte, and a single stub `@task` + `# TODO byte N.k:` line so it is **CI-green as-is** (owner/retries/tags gates pass). The learner fills in the body. README byte lines point at these files. (These practice scaffolds use the `s<N>_task<k>` name; a polished reference/demo may still be topic-named per R26.)
