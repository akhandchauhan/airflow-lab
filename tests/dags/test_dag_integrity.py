...
--------------------------------------------------------------------------
Two Airflow 3 gotchas that every Airflow 2 tutorial still gets wrong
--------------------------------------------------------------------------
1. `from airflow.models import DagBag` still "works", but it is a deprecation
   ...
   The file-parsing DagBag lives at `airflow.dag_processing.dagbag`:

       DagBag(dag_folder=None, safe_mode=NOTSET, load_op_links=True,
              collect_dags=True, known_pools=None, bundle_path=None,
              bundle_name=None)

   There is no `include_examples` any more - example DAGs are controlled by
   `AIRFLOW__CORE__LOAD_EXAMPLES` instead (set below, and in CI).

2. `DagBag.get_dag()` refreshes from the metadata DB. On any machine without a
   migrated database - CI, a fresh clone, a colleague's laptop - it fails with
   `OperationalError: no such table: dag`. Read `DagBag.dags[...]` instead,
   which is the plain in-memory result of parsing the files.
"""
...
# bundle_path is new in Airflow 3 - the base that import_errors keys and other
...
def test_dag_has_tags(dag_id: str) -> None:
    ...
    # DAG_BAG.dags[...] - NOT get_dag(), which needs a migrated metadata DB.
    assert DAG_BAG.dags[dag_id].tags, f"{dag_id} has no tags"
