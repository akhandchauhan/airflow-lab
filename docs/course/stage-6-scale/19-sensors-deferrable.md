...
wait_loader = ExternalTaskSensor(
    task_id="wait_loader",
    external_dag_id="s9_producer",       # the DAG we depend on
...
| **Asset** (Session 09) | producer *rings*, consumer scheduled on it | a shared **asset name**, not dag/task ids | **none** — no waiting task at all | the producer is an Airflow task you can add an outlet to |

The mental model: **if the thing you wait on is another Airflow task you control, use an asset (Session 09) — no polling, no date-matching, nothing waiting.** If it's *outside* Airflow (a file, an API, a partition) or an upstream you can't modify, use a sensor — and make it **deferrable** if a version exists, `reschedule` if not. Reserve plain `poke` for genuinely short waits.
...
```bash
python dags/stage-6-scale/s19/s19_examples.py
...
**File:** `dags/stage-6-scale/s19/s19_assignment.py` · **dag_id:** `s19_assignment`

Build a **cross-DAG wait** that consumes the Session 09 producer without polling a held slot.

**The problem:**

- One `ExternalTaskSensor` waits for the task `load_questions` in DAG `s9_producer` to succeed — running in **`reschedule`** mode (or `deferrable=True`) so it never holds a worker slot while waiting.
...
- `python dags/stage-6-scale/s19/s19_assignment.py` parses (prints nothing).
- With `s9_producer` present, `airflow dags test s19_assignment 2026-01-01` reaches the final task once the external task's run is found.
...
- **If you're waiting on your own upstream, don't wait at all** — an asset (Session 09) means zero polling tasks and no date-matching. The best sensor is the one you deleted.
...
```bash
python dags/stage-6-scale/s19/s19_assignment.py
