...
| `trigger_rule`            | when this task runs relative to upstream (Session 14) |
| `pool`, `priority_weight` | concurrency control (Session 10)                      |
...
```bash
python dags/stage-1-basic-dags/s2/classic_operators_demo.py
...
Create `dags/stage-1-basic-dags/s2/classic_operators.py`, dag_id `s2_classic_operators`. Build the
...
- `python dags/stage-1-basic-dags/s2/classic_operators.py` parses cleanly.
...
```bash
python dags/stage-1-basic-dags/s2/classic_operators.py
airflow dags test s2_classic_operators 2026-01-01
python -m pytest tests/ -v
git add dags/stage-1-basic-dags/s2/classic_operators.py && git commit -m "course: 02 classic operators" && git push
```
