...
```bash
python dags/stage-1-basic-dags/s5/s5_task1.py                                          # parses
airflow dags test s5_task1 2026-01-01                               # defaults → "hello world"
airflow dags test s5_task1 2026-01-01 --conf '{"name": "Panda", "times": 3, "shout": true}'
```

The last run prints `HELLO PANDA` three times. Now break a rule on purpose:

```bash
airflow dags test s5_task1 2026-01-01 --conf '{"times": 99}'       # maximum=10 → REJECTED
```
...
**File:** `dags/stage-1-basic-dags/s5/s5_task3.py` · **dag_id:** `s5_task3`
...
- `python dags/stage-1-basic-dags/s5/s5_task3.py` parses (prints nothing).
- `airflow dags test s5_task3 2026-01-01` prints the line with the **defaults**.
- `airflow dags test s5_task3 2026-01-01 --conf '{"country": "US", "limit": 5, "env": "prod"}'`
...
- **Params are visible — never pass a secret through one.** Param values show in the
  ...
  secrets backend (Session 12), never in a param.
...
```bash
python dags/stage-1-basic-dags/s5/s5_task3.py
