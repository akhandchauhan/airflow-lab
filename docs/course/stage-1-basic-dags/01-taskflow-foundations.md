...
Create `dags/stage-1-basic-dags/s1/taskflow.py`, dag_id `s1_taskflow`. A
...
```bash
python dags/stage-1-basic-dags/s1/taskflow.py                        # must parse cleanly
airflow dags test s1_taskflow 2026-01-01          # runs the whole DAG
python -m pytest tests/ -v                                    # integrity gates pass
git add dags/stage-1-basic-dags/s1/taskflow.py && git commit -m "course: 01 taskflow foundations" && git push
```
