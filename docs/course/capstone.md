...
| --- | --- | --- |
| **Trigger — assets** | the mart DAG runs when the raw-load DAG signals "today's data landed", not on a hopeful clock | Session 12 · Assets, Session 13 · Asset logic (`AssetAll`) |
| **Schedule floor** | a time floor so a dead producer can't mean a dead report (`AssetOrTimeSchedule`) | Session 09 · Schedules |
| **Params** | the run takes a target date (and top-N for tags) as validated input, so the same DAG backfills any day | Session 07 · Params |
| **Connections & hooks** | every BigQuery touch resolves `google_cloud_default` at run time; no secret in the file | Session 15 · Hooks |
| **Cost-capped ELT** | the transforms run *in* BigQuery via `BigQueryInsertJobOperator`, every query carrying `maximumBytesBilled`; Airflow only orchestrates and reads small scalars | Session 06 · BigQuery, P1 |
| **Branch on data** | after a freshness probe, choose a path — publish vs skip-and-alert on a stale/empty day | Session 04 · Branching & trigger rules |
| **Dynamic mapping** | fan the per-tag rollup out over the top-N tags decided at run time, then fan the results back in | Session 08 · Dynamic task mapping |
| **Quality gate** | a `BigQueryCheckOperator` (and a value/interval check) fails the run *before* the mart publishes if a metric is empty, null, or drifts implausibly | Session 06 §9, Session 16 · Data quality |
...
| **Idempotency** | the load overwrites exactly the target date's partition (`WRITE_TRUNCATE` on a partition / `MERGE`) so any retry or backfill lands the same rows | Session 06 §4 |
...
Every query in the capstone obeys the same rules P1 and Session 06 drilled, because one uncapped scan is a budget incident, not a style nit:
...
- One (or two) tasks that stand in for "today's raw questions/answers landed" — for the capstone they may be capped read-only probes on `posts_questions` / `posts_answers` for the target date — that **`outlets`** the raw asset(s). Success writes the asset event (Session 12).

**Consumer DAG — `capstone_assignment`** (the mart):

1. **Scheduled on the asset(s)** with `schedule=(questions_raw & answers_raw)` (`AssetAll`, Session 13), wrapped so there is also a **daily time floor** (`AssetOrTimeSchedule`, Session 09) — the report still runs once a day even if a producer dies.
2. **Params (Session 07):** `target_date` (string, defaults to your in-data anchor) and `top_n` (integer, `minimum=1`, default e.g. 10). Read them via `get_current_context()["params"]`. Bad input is rejected at trigger.
3. **Freshness probe + branch (Session 04):** a task reads a small scalar (row count for `target_date`) through a **hook** (Session 15); a `@task.branch` returns the **task_id** of either the publish path or a `skip_and_note` path when the day is empty/stale.
...
5. **Dynamic-mapped per-tag health (Session 08):** a task returns the **top-N tags** (from the tiny `tags` table or a capped query) at run time; a mapped task computes each tag's answer rate with `.partial()` freezing the constants and `.expand()` over the tag list; a **reduce** task fans the per-tag results back into one structure.
...
8. **Slack alerting (Session 17):** set `on_failure_callback=[send_slack_notification(...)]` (from `airflow.providers.slack.notifications.slack`) at the DAG or task level so any failure pages on-call. Requires a `slack_default` Connection (Session 15).
...
- All credentials come from Connections resolved at run time; **no secret grep-able in the files** (Session 15). Hooks instantiated **inside** tasks, never at module top level or the `@dag` body.
- The consumer references the **assets**, never the producer's dag_id (Session 12).
- Loads are **idempotent** — a rerun of any date lands identical rows (Session 06).
