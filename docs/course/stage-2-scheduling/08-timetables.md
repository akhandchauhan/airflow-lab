...
**Rule of thumb:** if the task cares about a *time window of data* (Session 06's incremental pattern, Session 07's idempotent backfill), you want a **data-interval** timetable so `[data_interval_start, data_interval_end)` is a real span. If the task just needs to *fire at a time* and doesn't slice data by interval, `CronTriggerTimetable` is simpler and its label matches the wall clock — no "why does my midnight run say yesterday" confusion.
...
| `AssetOrTimeSchedule` | `airflow.timetables.assets` | run on an asset event **or** on a time schedule (Session 09's floor) |
...
```bash
python dags/stage-2-scheduling/s8/s8_examples.py                  # parses (prints nothing)
airflow dags test s8_examples 2026-01-01        # prints start == end == the fire time
```
...
**File:** `dags/stage-2-scheduling/s8/s8_assignment.py`  ·  **dag_id:** `s8_assignment`
...
- `python dags/stage-2-scheduling/s8/s8_assignment.py` parses (prints nothing).
- `airflow dags test s8_assignment 2026-01-02` (a Friday) runs and prints a weekday.
...
```bash
python dags/stage-2-scheduling/s8/s8_assignment.py
airflow dags test s8_assignment 2026-01-02
python -m pytest tests/ -v
git add -A && git commit -m "session 11: timetables" && git push
```

Done when the custom timetable produces weekday-only runs and never a weekend one. Tick Session 08 in `docs/course/README.md`.
