...
`catchup` is the paper boy dumping the whole pile of unread papers on your porch the moment you get home (Session 06's "loaded gun"). Backfill is you walking to the archive and pulling exactly the issues from March 3rd to March 9th. One is automatic and unbounded; the other is manual, bounded, and re-runnable.
...
```bash
python dags/stage-2-scheduling/s7/s7_examples.py                 # parses (prints nothing)
airflow dags test s7_examples 2026-01-01       # one run: window 2026-01-01 → 2026-01-02
...
**File:** `dags/stage-2-scheduling/s7/s7_assignment.py`  ·  **dag_id:** `s7_assignment`
...
- `python dags/stage-2-scheduling/s7/s7_assignment.py` parses (prints nothing).
- `airflow dags test s7_assignment 2026-01-03` logs a value tied to Jan 3.
- `airflow backfill create --dag-id s7_assignment --from-date 2026-01-01 --to-date 2026-01-05 --reprocess-behavior none` creates the missing runs.
...
```bash
python dags/stage-2-scheduling/s7/s7_assignment.py
airflow dags test s7_assignment 2026-01-03
airflow backfill create --dag-id s7_assignment --from-date 2026-01-01 --to-date 2026-01-05
python -m pytest tests/ -v
git add -A && git commit -m "session 10: backfill" && git push
```

Done when a `completed` reprocess of the same range produces the identical output it did the first time. Tick Session 07 in `docs/course/README.md`.
