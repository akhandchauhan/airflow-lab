# Session 14 · Event-driven scheduling (the ring comes from outside Airflow)
...
Session 12's rule was blunt and worth repeating: **Airflow does not watch your data — it watches whether a task succeeded.** An asset event is written when a task with that asset in `outlets` finishes. That's the doorbell: *your* task rings it.
...
An SQS-watched asset and a consumer that reads the message payload. Unlike §12–13 this **cannot fully "run" from `dags test`** — a real message on a real queue is what fires it — so the reference is the **deployable shape**; type it into `dags/s14/s14_examples.py`. It parses clean and passes integrity gates as-is.
...
```bash
python dags/s14/s14_examples.py     # parses; asset shows a watcher in the UI Assets view
# to actually fire it: run a triggerer, define the SQS connection, and drop a message on the queue
```

To see it fire you need three things live: a **running triggerer**, the queue **connection** defined (e.g. `aws_default` for SQS), and an actual **message** on the queue. Then the watcher turns that message into an asset event and `s14_examples` runs — with no producer DAG anywhere.
...
**File:** `dags/s14/s14_assignment.py` · **dag_id:** `s14_assignment`
...
- Schedule `s14_assignment` on an **`AssetOrTimeSchedule`**: `assets=` your watched asset, `timetable=` a `CronTriggerTimetable` daily tick.
...
- Passes integrity gates: non-empty `tags`, real `owner`, `retries >= 1`, and **parses clean** (`python dags/s14/s14_assignment.py`) with no live queue.

**Acceptance criteria:**

- `python dags/s14/s14_assignment.py` parses with no errors.
- The UI **Assets** view shows the asset **with a watcher** and `s14_assignment` as its consumer.
