...
Session 16 gave you a circuit breaker that trips on bad data. But a tripped breaker in an empty house helps no one — someone has to *hear* it. Retries (Session 04) buy you automatic recovery; when the retries are exhausted and the task is finally, truly failed, the pipeline needs to **reach a human**. Without that, the standard failure mode is: the run goes red in a UI nobody is watching, the on-call finds out at 9am when the dashboard is stale, and the post-mortem question is always the same — *"why didn't we know sooner?"*
...
- **Every callback receives the `context`** — the same run-context mapping you get from `get_current_context()` (Session 06), carrying `task_instance`, `dag_run`, `logical_date`, and the exception. A minimal callback:
...
```python
# dags/stage-5-quality/s17/s17_examples.py
...
```bash
python dags/stage-5-quality/s17/s17_examples.py
...
**File:** `dags/stage-5-quality/s17/s17_assignment.py` · **dag_id:** `s17_assignment`
...
- No secret in the file — a real Slack alert reads `slack_conn_id`, never a hardcoded token (Session 12).
...
- `python dags/stage-5-quality/s17/s17_assignment.py` parses (prints nothing).
