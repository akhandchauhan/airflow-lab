...
|---|---|---|
| **Parse** | `python dags/stage-0-day-one/s0/s0_examples.py` | import errors, syntax, undefined names (prints nothing = OK) |
...
if __name__ == "__main__":
    dag_obj.test()          # run it straight from python: `python dags/stage-0-day-one/s0/s0_examples.py`
```

```bash
python dags/stage-0-day-one/s0/s0_examples.py            # with the __main__ block, this RUNS it via .test()
...
**File:** `dags/stage-0-day-one/s0/s0_assignment.py`  ·  **dag_id:** `s0_assignment`
...
- `python dags/stage-0-day-one/s0/s0_assignment.py` parses (prints nothing without the `__main__` block).
...
```bash
python dags/stage-0-day-one/s0/s0_assignment.py
...
`docs/course/README.md`, and skim the [Stage 1 recap](../stage-1-basic-dags/stage1-recap.md) to see where
this leads.
