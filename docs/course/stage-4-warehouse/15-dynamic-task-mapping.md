# Session 15 · Dynamic Task Mapping
...
@dag(
    ...
    schedule=None,
    catchup=False,
    tags=["session-15", "dynamic-mapping"],
...
```bash
python dags/stage-4-warehouse/s15/s15_examples.py
airflow dags test s15_examples 2026-01-01
```
...
**File:** `dags/stage-4-warehouse/s15/s15_assignment.py`  ·  **dag_id:** `s15_assignment`
...
- `python dags/stage-4-warehouse/s15/s15_assignment.py` parses (prints nothing).
- The Graph shows the mapped task as **[N]** with one instance per region, and the
  reduce task running once.
- `airflow dags test s15_assignment 2026-01-01` runs green.
...
- **Each instance must be idempotent on its own.** A mapped instance can retry alone
  (Session 04). If `process(item=x)` appends or writes, a retry of just that instance
...
```bash
python dags/stage-4-warehouse/s15/s15_assignment.py
airflow dags test s15_assignment 2026-01-01
...
Session 15 in `docs/course/README.md`.
