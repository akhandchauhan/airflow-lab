# Session 14 · Give the Pipeline a Brain
...
```python
# dags/stage-4-warehouse/s14/product_health_demo.py
...
@dag(
    dag_id="s14_product_health_demo",
    start_date=pendulum.datetime(2026, 1, 1, tz="UTC"),
    schedule=None,
    catchup=False,
    tags=["session-14", "branching", "stackoverflow"],
...
```bash
python dags/stage-4-warehouse/s14/product_health_demo.py
airflow dags test s14_product_health_demo 2026-01-01
```
...
**File:** `dags/stage-4-warehouse/s14/product_health.py` · **dag_id:** `s14_product_health`
...
| 4a | `tag_breakdown` | `BigQueryInsertJobOperator` | top-10 tags among those popular questions (`UNNEST` the tags — see [unnest](../reference/unnest.md))   |
...
| Check | Points |
| ----- | ------ |
| `python dags/stage-4-warehouse/s14/product_health.py` parses; `python -m pytest tests/ -v` green | 4 |
| `airflow dags test s14_product_health 2026-01-01` runs green end-to-end | 4 |
...
```bash
python dags/stage-4-warehouse/s14/product_health.py
airflow dags test s14_product_health 2026-01-01
