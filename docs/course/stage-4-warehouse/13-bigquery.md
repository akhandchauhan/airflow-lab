# Session 13 · Inside the Warehouse
...
result depends on *when* it ran. Same idea as retries in Session 04: a task must be safe
to run twice.
...
For a small scalar back in Python, use `BigQueryHook.get_first` in a `@task` (Session 12).
...
```python
# dags/stage-4-warehouse/s13/s13_task1.py
...
```bash
python dags/stage-4-warehouse/s13/s13_task1.py
airflow dags test s13_task1 2026-01-01
```
...
**File:** `dags/stage-4-warehouse/s13/s13_task2.py` (scaffold ready) · **dag_id:** `s13_task2`
...
- `python dags/stage-4-warehouse/s13/s13_task2.py` parses (prints nothing).
- `airflow dags test s13_task2 2026-01-01` runs green.
...
- **A non-idempotent load turns a retry into corrupted data.** If a task appends rows
  and then Airflow retries it (Session 04), you've now double-counted. Use
...
```bash
python dags/stage-4-warehouse/s13/s13_task2.py
airflow dags test s13_task2 2026-01-01
...
**Downstream:** the raise marks the task **failed** → your `retries` (Session 04) kick
