# Session 12 · The Universal Adapter
...
- **Operators are built out of hooks.** `BigQueryInsertJobOperator` uses
  ...
  (as you already did in Session 14), you're doing exactly what the operator does
...
```python
# dags/stage-4-warehouse/s12/answer_rate_demo.py
...
@dag(
    dag_id="s12_answer_rate_demo",
    start_date=pendulum.datetime(2026, 1, 1, tz="UTC"),
    schedule=None,
    catchup=False,
    tags=["session-12", "hooks", "stackoverflow"],
...
```bash
python dags/stage-4-warehouse/s12/answer_rate_demo.py
airflow dags test s12_answer_rate_demo 2026-01-01
```
...
> (`maximumBytesBilled`) — Session 14 — and read the result table, rather than
...
**File:** `dags/stage-4-warehouse/s12/answer_rate.py` · **dag_id:** `s12_answer_rate`
...
- `python dags/stage-4-warehouse/s12/answer_rate.py` parses (prints nothing).
- `airflow dags test s12_answer_rate 2026-01-01` runs green.
...
```bash
mkdir -p dags/stage-1-basic-dags/s4
python dags/stage-4-warehouse/s12/answer_rate.py
airflow dags test s12_answer_rate 2026-01-01
